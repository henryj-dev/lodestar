...
- [ ] `bun run check` · `bun run lint` · `bun test` 전부 통과
- [ ] 권한 미정의 기존 RP 의 토큰 페이로드 무회귀(2-2 수용 기준)
- [ ] SET 의 `roles` 키가 기존 형태 그대로(3-1 수용 기준)
- [ ] 마이그레이션은 생성만, 적용은 사용자 실행
- [ ] 작성자와 분리된 독립 리뷰 통과
