...
### **Cloudflare Workers 위에서 도는 오픈소스 아이덴티티 프로바이더.**

OIDC, SAML 2.0, WebAuthn/Passkey, TOTP 2FA, LDAP 연동을 지원하고 멀티테넌트 조직
관리를 포함합니다. 하나의 SvelteKit 애플리케이션으로 배포되며, 데이터베이스 방언과
배포 타깃은 빌드 시점에 고릅니다.
...
> _길잡이 별(lodestar)은 방향을 잡을 때 보는 별입니다 — 다른 모든 것이 자기 위치를
> 재는 고정점._
...
아이덴티티 프로바이더는 결국 성격이 다른 두 가지를 들게 됩니다. 그 사람이 조직에서 누구인가,
그리고 각 서비스에서 무엇을 할 수 있는가. 하나로 둘 다 답하고 싶어집니다 — 조직도는 이미 있고,
이미 목록이고, 그룹 목록은 권한 집합처럼 보이니까요.

같은 것이 아닙니다. 그룹 소속으로 인가하면 팀 이동이 보안 경계를 움직이고, 조직 개편이 권한을
재배정하며, 조직도를 고치는 사람이 곧 권한을 주는 사람이 됩니다.

Lodestar 는 둘을 의도적으로 갈라 둡니다. `groups` 와 `organization` 계열 클레임은 인사 구조를
서술하며 표시용입니다. `roles` 와 `entitlements` 는 사용자별·서비스별로 배정되고 인가에 쓰이는
유일한 클레임입니다 — 서비스 배정이 있으면 scope 와 무관하게 발행되고, 관리자가 바꾸면 Security
Event Token 으로 RP 에 통지됩니다. SAML SP 는 이름으로 허용받은 조직 속성만 볼 수 있습니다.
...
- **Database**: Cloudflare D1(SQLite) / libSQL(SQLite) / PostgreSQL / MySQL 중 택1 (Drizzle ORM). PostgreSQL·MySQL 은 Hyperdrive · Workers VPC · `DATABASE_URL` 직결 중 선택 ([DB 방언 선택](#db-방언-선택) 참고)
...
**아직 안 되는 것.** 감사 로그의 MAC 은 행 단위 위변조는 잡지만 **행 삭제 자체는 탐지하지
못한다** — prev-hash 체인이 아니라 동시 쓰기 fork 가 없는 대신 치르는 값이다. 필요하면
Logpush 같은 외부 미러를 함께 써야 한다. `DISPATCHER_SERVICE_TOKEN` 은 전 스코프를 가진
레거시 경로로, 호출자를 전부 발급 토큰으로 옮긴 뒤 제거해야 한다.

**굳지 않았다.** 활발히 개발 중인 학습·실험 성격의 IdP 다. 프로덕션에 넣기 전에 자신의 위협
모델에 맞춘 보안 검토를 직접 하기를 권한다. 알려진 보안 한계는 [보안 참고사항](#보안-참고사항)에
정리돼 있다.
